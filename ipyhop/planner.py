...
class IPyHOP(object):
    ...
    *   planner = IPyHOP(methods, actions) tells IPyHOP to create a IPyHOP planner object.
    ...
    def select_next_open_node(
        ...
        # NEEDS CHANGES
        # time point ordering (new node type?, method that adds time point to ordering, semisort selection) x
        # time advancing (new node type?, method that changes t_now and adds temporal constraint for immediate x
        # prior time point in ordering, is there a principled way to do this?) x
        # change how nodes are selected for expansion x
        # change back track to restore temporal network x
        curr_node_id = None
        ...
        if self.is_temporal and value_chronicle is not None:
            ...
            for node_id in open_frontier_node_tup_filter:
                ...
                # for those maximum priority
                    ...
                    parent_node_id = next( sol_tree.predecessors( node_id ) )
        ...
        while True:
            ...
            # or to use the old (same as previous iteration) current node again
                ...
                if curr_node[ "type" ] in { "G", "T", "M" }:
                    ...
                    if curr_node[ "exhausted_methods" ]:
                        ...
                        if verbose > 2:
                            print(
                                    "Node " + str( curr_node_id ) + ", " + str(
                                            curr_node[ "info" ],
                                    ) + ", has exhausted methods for present current node",
                            )
            ...
            if need_new_curr_node:
                try:
                    ...
                    # print( prev_node )
                    backtrack( closed_node_backtrack=True )
                    # return to loop start
                    # I do not understand why this breaks the temporal mode and is needed for atemporal
                    # loop back to start for root to terminate planning as impossible
                    # if not is_temporal:
                    continue
            else:
                curr_node_id = cast( int, prev_node[ "next_node_id" ] )
    ...
    def _node_refine(
        ...
        #     print( 'CURR NODE REFINE START STATE' )
        #     print( self.state.t_ordered )
        #     print( self.state.t_unordered )
        if curr_node_id in node_id_visit_order:
        ...
        if is_temporal:
            ...
            if is_temporal:
                ...
                # toc_closed_unvisited_lst = [ ]
                # for node_id, time_point in toc_closed_id_node_tup_lst:
                #     if node_id not in node_id_visit_order:
                #         toc_closed_unvisited_lst.append( time_point )
                # print( toc_closed_unvisited_lst )
                # print( "  IN NETWORK" )
                # print( [ *value_chronicle.temporal_network.min_stn.nodes ] )
                # print( [ *value_chronicle.temporal_network.min_stn.edges.data() ] )
                assert set( self.state.t_unordered ) | set( self.state.t_ordered ) == set(
                        value_chronicle.temporal_network.min_stn.nodes,
                )
            ...
            if curr_node_info not in self.blacklist:
                ...
                if is_temporal:
                    ...
                    if temporal_action_output is not None:
                        ...
                        curr_node[ "temporal_restoration_tuple" ] = temporal_restoration_tup
        ...
        # If current node is a Goal
            ...
            if goal_done:
                ...
                if is_temporal:
                    ...
                    subgoals += toc_lst
                self._add_nodes_and_edges( curr_node_id, subgoals )
                self.state = new_state.copy()
                                    ...
                                    if reference_chronicle is not None:
                                        ...
                                        for subgoal in subgoals:
                                            ...
                                            # actions
                                                ...
                                                raise (ValueError( "Unsupported node in subgoals: " + str( subgoal ) ))
                                            for tp in tps:
                                                if all(
                                                        [
            ...
            if subgoals is None:
                if verbose > 2:
                    ...
                    print(
                            'Iteration {}, Backtracking to {}.'.format(
                                    _iter, repr( self.sol_tree.nodes[ node_id_visit_order[ -1 ] ][ 'info' ] ),
                            ),
                    )
                curr_node[ 'exhausted_methods' ] = True
                backtrack()
        ...
        # future clobbering
            ...
            parent_node_id = next( sol_tree.predecessors( curr_node_id ) )
            ...
            parent_node_info = self.sol_tree.nodes[ parent_node_id ][ 'info' ]
            if success_flag:
                if verbose > 2:
                    print( 'Iteration {}, Goal {} Verification successful.'.format( _iter, str( parent_node_info ) ) )
                curr_node[ 'status' ] = "C"
                node_id_visit_order.append( curr_node_id )
                self.state = new_state.copy()
            else:
                if verbose > 2:
                    print( 'Iteration {}, Goal {} Verification failed.'.format( _iter, str( parent_node_info ) ) )
                    print(
                            'Iteration {}, Backtracking to {}.'.format(
                                    _iter, repr( self.sol_tree.nodes[ curr_node_id ][ 'info' ] ),
                            ),
        ...
        # adds time point to ordered_time_points and equivalent time constraint
            ...
            # print( "CURRENT SEPARATION LIST" )
            # print( curr_node[ 'seperation_condition_lst' ] )
            while curr_node[ 'seperation_condition_lst' ] != [ ] and not (success_flag):
                separation_condition = curr_node[ 'seperation_condition_lst' ].pop( 0 )
                if value_chronicle is not None:
                    ...
                    if success_flag:
                        ...
                        node_id_visit_order.append( curr_node_id )
                        self.state = new_state.copy()
                        assert temporal_restoration_tup is not None
                        curr_node[ "temporal_restoration_tuple" ] = temporal_restoration_tup
                        if verbose > 2:
                            print(
        ...
        # attempt to insert changes associated with temporal singleton action
            ...
            if value_chronicle is not None:
                ...
                if success_flag:
                    if verbose > 2:
                        print(
                                        'Iteration {}, TSA Node {}, succeeded'.format(
                                                _iter,
                                                repr( curr_node_info ),
                                        ),
                                ),
            ...
            if not success_flag:
                if verbose > 2:
                    print(
                            'Iteration {}, TSA Node {}, failed'.format(
                                    _iter,
                                    repr( curr_node_info ),
                            ),
                    )
    ...
    def _backtrack(self, closed_node_backtrack=False):
        print( "ROLLBACK" )
        # localization
        is_temporal = self.is_temporal
        value_chronicle = self.value_chronicle
        ...
        # need to reopen previous node
        print( "PREV NODE" )
        print( prev_node[ "info" ] )
        print( "FAILED NODE ID" )
        print( curr_node_id )

        if curr_node_id is None:
            print( "CLOSED BACKTRACK" )
            ...
            # # # reset fields that are type-specifc
            # if prev_node[ 'type' ] in [ 'G', 'M', 'T' ]:
            #     relevant_methods = prev_node[ 'methods' ]
            ...
            # reset current node to state at creation
            curr_node = sol_tree.nodes[ curr_node_id ]
            print( "FAILED NODE" )
            print( curr_node[ "info" ] )
            # if is_temporal:
            ...
            # rollback changes to value chronicle
